...
const showRecipe = function() {
    ...
    fetch("https://forkify-api.herokuapp.com/api/v2/recipes/5ed6604591c37cdc054bc886").then((res)=>{
        if (!res.ok) throw new Error(`${res.status}`);
        console.log(res);
        return res.json();
    }).then((data)=>{
        console.log(data);
        const { id , title , publisher , source_Url: sourceUrl , image_url: image , cooking_time: cookingTime , ingredients , servings  } = data.data.recipe;
        console.log(title);
        //2) Rendering recipe
